"""Stage S: Harris-only staging archive pilot."""
...
from corpus_stage_r_harris_fileinfo_attach import MANAGED_FILENAME, MANAGED_RELATIVE_PATH
...
def _archive_destination() -> Path:
    base = (config.STAGING / "processed" / HARRIS_ID_PREVIEW).resolve()
    dest = (base / HARRIS_NAME).resolve()
...
def _harris_db_row() -> dict[str, Any] | None:
    conn = sqlite3.connect(f"file:{config.DB_PATH}?mode=ro", uri=True)
    conn.row_factory = sqlite3.Row
    try:
...
def _acquire_lock(run_dir: Path) -> Path:
    ...
    payload = json.dumps(
        {
            "stage": "live-harris-staging-archive-only",
            "run_id": run_dir.name,
            "created_at": _now(),
            "pid": os.getpid(),
        },
...
def _future_commands(run_id: str) -> list[str]:
    return [
        ...
        f"DO NOT RUN YET: python pipeline\\run_corpus_pipeline.py --execute --mode A --max-papers 5 --run-id {run_id} --execute-stage final-pilot-report-preview",
    ]
...
def _preconditions(plan: dict[str, Any], run_dir: Path) -> tuple[list[str], dict[str, Any]]:
    ...
    if not stage_q.get("post_copy_validation", {}).get("ok"):
        failures.append("Stage Q artifact does not show successful Harris PDF copy")
    if not stage_r.get("post_write_validation", {}).get("ok"):
        failures.append("Stage R artifact does not show successful file_info attachment")
    if not _db_row_attached(db_row):
        failures.append("Harris DB row does not point to the attached managed PDF")
...
def run_live_harris_staging_archive_only(plan: dict[str, Any], run_dir: Path) -> dict[str, Any]:
    ...
    if archive_hash != HARRIS_HASH:
        validation_failures.append("archive file hash mismatch")
    if managed_hash_after != HARRIS_HASH:
    ...
    summary = {
        ...
        "pdf_validation": {
            ...
            "delta": delta,
            "only_harris_staged_source_moved": delta == {"added": [archive_rel], "removed": [staged_rel], "changed": []},
            "no_pdfs_deleted": bool(delta.get("added")) and bool(delta.get("removed")) and not delta.get("changed"),
            "managed_pdf_unchanged": managed_hash_after == ctx["managed_hash"] == HARRIS_HASH,
        },
...
def _write_reports(run_dir: Path, summary: dict[str, Any]) -> None:
    (run_dir / "live_harris_staging_archive_only.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")
    (run_dir / "stage_live_harris_staging_archive_only_report.txt").write_text(render_live_harris_staging_archive_report(summary), encoding="utf-8")


def render_live_harris_staging_archive_report(summary: dict[str, Any]) -> str:
    dbv = summary.get("db_validation") or {}
    pdfv = summary.get("pdf_validation") or {}
    lines = ["Stage S Live Harris Staging Archive Only", "=" * 60]
    ...
    lines.append(f"Only Harris staged source moved: {pdfv.get('only_harris_staged_source_moved')}")
