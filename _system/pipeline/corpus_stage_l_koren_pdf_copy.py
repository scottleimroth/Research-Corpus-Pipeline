"""Stage L: Koren-only managed PDF copy pilot."""
...
def _read_stage_k(run_dir: Path) -> dict[str, Any]:
    path = run_dir / "live_koren_parent_record_only.json"
    if not path.exists():
        raise RuntimeError(f"Stage K artifact missing: {path}")
    return json.loads(path.read_text(encoding="utf-8"))


def _get_koren_db_row() -> dict[str, Any] | None:
...
def _safe_destination() -> Path:
    dest = (config.SOURCE_PDFS / f"{KOREN_ID}.pdf").resolve()
...
def _acquire_lock(run_dir: Path) -> Path:
    ...
    payload = json.dumps(
        {
            "stage": "live-koren-pdf-copy-only",
            "run_id": run_dir.name,
            "created_at": _now(),
            "pid": os.getpid(),
        },
...
def run_live_koren_pdf_copy_only(plan: dict[str, Any], run_dir: Path) -> dict[str, Any]:
    failures: list[str] = []
    stage_k = _read_stage_k(run_dir)
    if not (stage_k.get("row_created") and stage_k.get("row_id") == KOREN_ID):
        failures.append("Stage K artifact does not show the Koren parent row was created")
    pdf_plan = _koren_plan(plan)
    source = Path(pdf_plan["path"])
    if not source.exists():
        failures.append(f"Staged Koren PDF missing: {source}")
    source_hash = _sha256(source) if source.exists() else ""
    if source_hash != KOREN_HASH or source_hash != pdf_plan.get("sha256"):
        failures.append("Staged Koren PDF hash does not match Stage K/run plan hash")
    db_row = _get_koren_db_row()
    if not db_row:
        failures.append(f"{KOREN_ID} does not exist in papers.db")
    elif db_row.get("id") != KOREN_ID or db_row.get("document_type") != "book":
        failures.append("Koren DB row did not read back as expected")
    ...
    summary = {
        ...
        "db_update": {
            ...
            "reason": "copy-only pilot; existing Stage K row already records managed destination as preview-only",
            "before": before_db,
            "after": after_db,
        ...
        "pdf_validation": {
            ...
            "delta": delta,
            "only_koren_pdf_copied": copied and delta["added"] == [expected_added] or already_copied,
...
def _write_reports(run_dir: Path, summary: dict[str, Any]) -> None:
    (run_dir / "live_koren_pdf_copy_only.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")
    (run_dir / "stage_live_koren_pdf_copy_only_report.txt").write_text(render_live_koren_pdf_copy_report(summary), encoding="utf-8")


def render_live_koren_pdf_copy_report(summary: dict[str, Any]) -> str:
    lines = ["Stage L Live Koren PDF Copy Only", "=" * 50]
